...
function renderCreatorInterface() {
  renderView(`
      <div style="display: flex; flex-direction: row;" id="gameOverlay" >
        <div id="question" style="display: flex; flex-direction: column; width:25%; margin:20px 5px;">
...
renderView(`
  ...
  <form action="" class="form-main" style="margin-top: 20px; border-radius: 10px;min-height: 200px;">
    <div class="div-main question">
        ${questionData.answers.map((answer, index) => `
...
function updateLobbyGame() {
  ...
  if (userList) {
    ...
    state.users.forEach(user => {
      const imageUrl = `../Images/Player/${user.image}`;
      userList.innerHTML += `
        <div id="player-${user.id}" style="display: flex; flex-direction: column; flex-wrap: nowrap; width: 90px; margin: 5px 0; transition: transform 1s;">
...
socket.on('quiz-ended', (roomId)=> {
  if(state.isCreator){
    ...
    winnerOverlay.setAttribute("class", "fade-in");
...
`;
const imageUrl = `../Images/Player/${winner.image}`;
    winnerOverlay.innerHTML+=` <div id="player-${winner.id}" class="winner" style="display: flex; flex-direction: column; flex-wrap: nowrap; width: 90px; margin: 5px 0; transition: transform 1s;">
          <div style="margin: 0 15px; color: white; text-align:center;">
            <span style="text-align:center;">${winner.nickname}</span>
          </div>
          <img src="${imageUrl}" alt="" style="border: 1px solid transparent; border-radius: 15px; width: 90px; height: 70px; object-fit: cover;">
        </div>`;
}, 3000)
